...
public class DocumentService {
    ...
    public TreeNode createDocuments() {
        ...
        TreeNode root = new DefaultTreeNode(new PurchaseContract("ALP-20.3.xxxxx-P", "P1020", 0, data), null);

        List<PurchaseContract> pcontracts = new NfListDao().PContracts();
        for (PurchaseContract contract : pcontracts) {
            TreeNode purchasecontracts = new DefaultTreeNode(
                    new PurchaseContract(
                    contract.getPurchasectr(),
                    contract.getProductobject(),
                    contract.getCtrvolume(),
                    contract.getExpirydate()), root);
        }
